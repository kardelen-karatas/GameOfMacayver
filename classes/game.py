...
class Game:
    """
    Create the graphical environment of the game.
    It takes as a parameter a text file that contains the pattern
    Raise:
        IndexError: if the pattern does not
        composed of 15 lines and 15 columns
        ...
        try:
            ...
            self.display_surface = pygame.display.set_mode(
                (
                    ...
                    self.height * self.tile_size + self.tile_size)
            )
        ...
        try:
            for line in range(self.height):
                for column in range(self.width):
                    ...
                    if tile.lab_item:
                        self.display_surface.blit(
                            self.item_images[tile.lab_item.item_type],
                            (column * self.tile_size, line * self.tile_size)
                        )
        except IndexError:
    ...
    def display_text(self, text, color, text_place):
        ...
        Args :
            ...
            text_place (tuple): the coordinates in pixels of
            ...
            for event in pygame.event.get():
                if event.type == QUIT:
                    ...
                    if (
                        ...
                        self.player.x < self.width - 1 and
                        iswalkable(self.player.y, self.player.x + 1)
                    ):
                        self.player.pick_up(
